...
export class BufferPool {
	...
	constructor(init: BufferPoolInit) {
		...
		this.#max = max;
		this.#buckets = [[], [], []];
		this.#maxPerBucket = options?.maxPerBucket ??
			DefaultBytesPoolOptions.maxPerBucket;
		this.#maxTotalBytes = options?.maxTotalBytes ??
			DefaultBytesPoolOptions.maxTotalBytes;
		this.#currentBytes = 0;
	}
	...
	release(bytes: ArrayBufferLike): void {
		...
		if (
			this.#maxTotalBytes && this.#currentBytes + size > this.#maxTotalBytes
		) return;
		const bucket = this.#buckets[idx]!;
