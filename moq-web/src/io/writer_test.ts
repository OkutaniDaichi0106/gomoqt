...
describe('Writer', () => {
  ...
  describe('writeUint8Array', () => {
    ...
    it('should throw error for data exceeding maximum length', () => {
      ...
      assertThrows(() => { writer.writeUint8Array(largeData);
       }, Error, 'Bytes length exceeds maximum limit');
    });
  ...
  describe('writeBigVarint', () => {
    it('should write single byte varint for values < 255', async () => {
      ...
      expect(writtenData).toHaveLength(1);
      assertEquals(writtenData[0], new Uint8Array([42]));
    });
    ...
    it('should write two byte varint for values < 16384', async () => {
      ...
      assertEquals(written[0], 0x41); // 0x40 | (300 >> 8)
      assertEquals(written[1], 0x2C); // 300 & 0xFF
    });
    ...
    it('should write four byte varint for values < 2^30', async () => {
      ...
      assertEquals(written[0] & 0xC0, 0x80); // Check first 2 bits
    });
    ...
    it('should throw error for negative values', () => {
      assertThrows(() => { writer.writeBigVarint(-1n);
       }, Error, 'Varint cannot be negative');
    });

    it('should throw error for values exceeding maximum', () => {
      const maxValue = (1n << 62n) - 1n;
      assertThrows(() => { writer.writeBigVarint(maxValue + 1n);
       }, Error, 'Value exceeds maximum varint size');
    });
  });
  ...
  describe('writeBoolean', () => {
    it('should write true as 1', async () => {
      ...
      expect(writtenData).toHaveLength(1);
      assertEquals(writtenData[0], new Uint8Array([1]));
    });
    ...
    it('should write false as 0', async () => {
      ...
      expect(writtenData).toHaveLength(1);
      assertEquals(writtenData[0], new Uint8Array([0]));
    });
  });
