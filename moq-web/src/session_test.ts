...
Deno.test("Session acceptAnnounce", testOptions, () => {
  ...
  mockWebTransport.getBiReader().enqueue({ readable, writable } as WebTransportBidirectionalStream);
...
Deno.test("Session closeWithError closes underlying connection with code", testOptions, async () => {
  const mockWebTransport = new MockWebTransport();
  mockWebTransport.markReady();
  const session = new Session({ conn: mockWebTransport });
