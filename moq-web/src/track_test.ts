...
Deno.test("Track", async (t) => {
	await t.step(
		...
		async () => {
			...
			// Mock openUniStream
			const ms = new MockStream(77n);
			const openUni = async () => [ms.writable as any, undefined] as any;
	...
	await t.step(
		...
		async () => {
			...
			// Create an open uni stream for group
			const ms = new MockStream(200n);
			const openUni = async () => [ms.writable as any, undefined] as any;
			const tw = new TrackWriter("/t" as any, "test", sss, openUni);
