...
import { background, Context, withCancelCause } from "../deps.ts";
...
describe("TrackMux", () => {
	...
	describe("announce", () => {
		it("should register a handler for the announcement path", () => {
			...
			expect(mockHandler.serveTrack).toHaveBeenCalledWith(
				background().done(),
				mockTrackWriter,
			);
		});
		...
		it("should clean up handler when announcement ends", async () => {
			...
			expect(differentTrackWriter.closeWithError).toHaveBeenCalledWith(
	...
	describe("publish", () => {
		it("should create announcement and register handler", async () => {
			...
			const announcementSpy = vi.spyOn(Announcement.prototype, "constructor" as any)
	...
	describe("serveTrack", () => {
		...
		it("should call NotFoundHandler for unregistered path", async () => {
			...
			// Should call closeWithError for not found path
			expect(trackWriterWithDifferentPath.closeWithError).toHaveBeenCalledWith(
				TrackNotFoundErrorCode,
				"Track not found",
			);
		});
	...
	describe("serveAnnouncement", () => {
		...
		it("should clean up announcer when context ends", async () => {
			...
			const servePromise = trackMux.serveAnnouncement(
				mockAnnouncementWriterWithContext,
				validPrefix,
			);
