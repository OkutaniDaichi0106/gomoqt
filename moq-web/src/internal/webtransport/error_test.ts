...
Deno.test("webtransport/error - StreamError behavior", async (t) => {
	...
	await t.step("prototype chain and instanceof", () => {
		...
		assertEquals(error instanceof WebTransportStreamError, true);
		assertEquals(error instanceof Error, true);
		assertEquals(Object.getPrototypeOf(error), WebTransportStreamError.prototype);
	...
	await t.step("message handling", () => {
		const error1 = new WebTransportStreamError({ source: "stream", streamErrorCode: 1 }, false);
		assertEquals(error1.message, `stream was reset with code 1`);
		assertEquals(error1.code, 1);

		const error2 = new WebTransportStreamError({ source: "stream", streamErrorCode: 2 }, false);
		assertEquals(error2.message, `stream was reset with code 2`);
		const error3 = new WebTransportStreamError({ source: "stream", streamErrorCode: 3 }, false);
	...
	await t.step("throwing and catching", () => {
		...
		assertThrows(() => {
			throw new WebTransportStreamError({ source: "stream", streamErrorCode: code }, false);
		});
	...
	await t.step("serialization", () => {
		const error = new WebTransportStreamError({ source: "stream", streamErrorCode: 123 }, true);
