...
Deno.test("SessionStream", async (t) => {
  await t.step(
    "constructor initializes clientInfo and serverInfo correctly",
    async () => {
  ...
  await t.step(
    "handleUpdates updates serverInfo.bitrate on receiving update",
    async () => {
      const [ctx] = withCancelCause(background());
  ...
  await t.step(
    "waitForBackgroundTasks waits for all tasks to complete",
    async () => {
      const [ctx] = withCancelCause(background());
      ...
      const mockReadable = new MockReceiveStream({
        id: 6n,
        read: spy(async () =>
          [0, new EOFError()] as [number, Error | undefined]
        ),
      });
  ...
  await t.step(
    "clientInfo and serverInfo extensions are accessible",
    async () => {
      const [ctx] = withCancelCause(background());
