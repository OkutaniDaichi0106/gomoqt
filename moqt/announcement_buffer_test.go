...
func TestNewAnnouncementBuffer(t *testing.T) {
	...
	assert.NotNil(t, buffer.announced)
...
func TestAnnouncementsBuffer_SendAnnouncements(t *testing.T) {
	tc := map[string]func(t *testing.T){
		...
		"send announcements successfully": func(t *testing.T) {
			...
			assert.Len(t, buffer.announced, 1)
			assert.Equal(t, announcement, buffer.announced[0])
		},
		...
		"send empty announcements": func(t *testing.T) {
			...
			assert.Len(t, buffer.announced, 0)
		},
		...
		"send additional announcements": func(t *testing.T) {
			...
			assert.Len(t, buffer.announced, 2)
			assert.Equal(t, announcement1, buffer.announced[0])
			assert.Equal(t, announcement2, buffer.announced[1])
		},
...
func TestAnnouncementsBuffer_ServeAnnouncements(t *testing.T) {
	tc := map[string]func(t *testing.T){
		...
		"serve announcements successfully": func(t *testing.T) {
			...
			wg.Add(1)
			go func() {
				buffer.serveAnnouncements(writer)
			}()
...
func TestAnnouncementsBuffer_DeliverAnnouncements(t *testing.T) {
	tc := map[string]func(t *testing.T){
		"deliver an active announcement": func(t *testing.T) {
			t.Parallel()
			// Initialize the buffer
			buffer := newAnnouncementsBuffer()
			defer buffer.Close()

			delivered := make(map[TrackPath]*Announcement)
				...
				writer := &MockAnnouncementWriter{
					SendAnnouncementsFunc: func(announcements []*Announcement) error {
						for _, announcement := range announcements {
							delivered[announcement.TrackPath()] = announcement
						}
						return nil
					},
				}
			...
			// We expect exactly 1 announcements because:
			// 1. The first batch of unique announcements (announcement1)
			assert.Len(t, delivered, 1, "Expected delivery of all unique announcement instances")
			...
			assert.Equal(t, announcement1, delivered["/test/path1"], "First announcement should be delivered first")
			...
			assert.True(t, delivered["/test/path1"].IsActive(), "Original announcement1 should remain active")
		},
		"deliver an ended announcement": func(t *testing.T) {
			t.Parallel()
			// Initialize the buffer
			buffer := newAnnouncementsBuffer()
			defer buffer.Close()
			...
			// - announcement2: Marked inactive because a new instance with the same path was processed
			// - announcement2_dup: Remains active as it's the most recent instance for its path
			assert.False(t, delivered["/test/path1"].IsActive(), "Original announcement1 should remain active")
		},
		"deliver multiple unique announcements": func(t *testing.T) {
			t.Parallel()
			// Initialize the buffer
			buffer := newAnnouncementsBuffer()
			defer buffer.Close()
			...
			assert.Len(t, delivered, 2, "Expected delivery of all unique announcement instances")

			// Verify the delivery order matches the send order, which is critical for clients
			// that rely on receiving announcements in the correct sequence
			assert.Equal(t, announcement1, delivered["/test/path1"], "First announcement should be delivered first")
			assert.Equal(t, announcement2, delivered["/test/path2"], "Second announcement should be delivered second")
			...
			// - announcement2_dup: Remains active as it's the most recent instance for its path
			assert.True(t, delivered["/test/path1"].IsActive(), "Original announcement1 should remain active")
			assert.True(t, delivered["/test/path2"].IsActive(), "Original announcement2 should be inactive after duplicate was processed")
		},
		"deliver some duplicate announcements": func(t *testing.T) {
			...
			announcement1 := NewAnnouncement(TrackPath("/test/path1"))
			announcement2 := NewAnnouncement(TrackPath("/test/path2"))
			announcement3 := NewAnnouncement(TrackPath("/test/path3"))
			...
			// that rely on receiving announcements in the correct sequence
			assert.Equal(t, announcement1, delivered["/test/path1"], "First announcement should be delivered first")
			assert.Equal(t, announcement2_dup, delivered["/test/path2"], "Second announcement should be delivered second")
			assert.Equal(t, announcement3, delivered["/test/path3"], "Third announcement should be delivered third")
			...
			// - announcement2: Marked inactive because a new instance with the same path was processed
			// - announcement2_dup: Remains active as it's the most recent instance for its path
			assert.True(t, announcement1.IsActive(), "Original announcement1 should remain active")
			assert.False(t, announcement2.IsActive(), "Original announcement2 should be inactive after duplicate was processed")
			assert.True(t, announcement2_dup.IsActive(), "Duplicate announcement2 should remain active")
			assert.True(t, announcement3.IsActive(), "Original announcement3 should remain active")
		},
	}
