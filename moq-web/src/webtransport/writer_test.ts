import { assertEquals, assertExists, assertThrows, fail } from "../../deps.ts";
...
function createWriter(): { writer: Writer; writtenData: Uint8Array[]; streamClosed: boolean } {
  ...
  const writableStream = new WritableStream<Uint8Array>({
    write(chunk) { writtenData.push(chunk); },
...
Deno.test('webtransport/writer - writeUint8Array', async (t) => {
  ...
  await t.step('throws for data exceeding maximum length', () => {
    ...
    assertThrows(() => { writer.writeUint8Array(largeData); }, Error, 'Bytes length exceeds maximum limit');
  });
...
Deno.test('webtransport/writer - writeBigVarint', async (t) => {
  ...
  assertEquals(written[0], 0xC0);
    ...
    out = createWriter();
    assertThrows(() => { out.writer.writeBigVarint(-1n); }, Error, 'Varint cannot be negative');

    out = createWriter();
    const maxValue = (1n << 62n) - 1n;
    assertThrows(() => { out.writer.writeBigVarint(maxValue + 1n); }, Error, 'Value exceeds maximum varint size');
  });
});
...
Deno.test('webtransport/writer - close/cancel/closed', async (t) => {
  await t.step('close behavior', async () => {
    ...
    try { await out.writer.close(); } catch (e) { assertExists(e); }
  });
