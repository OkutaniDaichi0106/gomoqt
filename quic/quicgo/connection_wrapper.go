...
func wrapConnection(conn *quicgo_quicgo.Conn) internal.Connection {
	if conn == nil {
		return nil
	}
	return &connWrapper{
...
var _ internal.Connection = (*connWrapper)(nil)

type connWrapper struct {
	conn *quicgo_quicgo.Conn
}

func (wrapper *connWrapper) AcceptStream(ctx context.Context) (internal.Stream, error) {
	stream, err := wrapper.conn.AcceptStream(ctx)
	return &rawQuicStream{stream: stream}, wrapError(err)
}

func (wrapper *connWrapper) AcceptUniStream(ctx context.Context) (internal.ReceiveStream, error) {
	stream, err := wrapper.conn.AcceptUniStream(ctx)
	return &rawQuicReceiveStream{stream: stream}, wrapError(err)
}

func (wrapper *connWrapper) CloseWithError(code internal.ConnectionErrorCode, msg string) error {
	err := wrapper.conn.CloseWithError(quicgo_quicgo.ApplicationErrorCode(code), msg)
	return wrapError(err)
}

func (wrapper *connWrapper) ConnectionState() internal.ConnectionState {
	...
	return internal.ConnectionState{
		...
		Version:           internal.Version(state.Version),
		GSO:               state.GSO,
	}
}
...
func (wrapper *connWrapper) OpenStream() (internal.Stream, error) {
	stream, err := wrapper.conn.OpenStream()
	return &rawQuicStream{stream: stream}, wrapError(err)
}

func (wrapper *connWrapper) OpenStreamSync(ctx context.Context) (internal.Stream, error) {
	stream, err := wrapper.conn.OpenStreamSync(ctx)
	return &rawQuicStream{stream: stream}, wrapError(err)
}

func (wrapper *connWrapper) OpenUniStream() (internal.SendStream, error) {
	stream, err := wrapper.conn.OpenUniStream()
	return &rawQuicSendStream{stream: stream}, wrapError(err)
}

func (wrapper *connWrapper) OpenUniStreamSync(ctx context.Context) (internal.SendStream, error) {
